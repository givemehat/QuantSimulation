...
def plot_price_with_signals(
    ...
    # Volume
    colors = [ACCENT if c >= o else ACCENT2 for c, o in zip(df["Close"], df["Open"])]
    fig.add_trace(
        go.Bar(
            x=df.index,
            y=df["Volume"],
...
def plot_feature_importance(
    feature_importances: pd.Series, top_n: int = 20
) -> go.Figure:
...
def plot_rolling_sharpe(equity_df: pd.DataFrame, window: int = 60) -> go.Figure:
    ...
    rolling_sharpe = (
        ret.rolling(window).mean() / (ret.rolling(window).std() + 1e-9)
    ) * np.sqrt(252)
